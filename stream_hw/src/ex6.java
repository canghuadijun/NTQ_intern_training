...
public class ex6 {
    public static void main(String[] args) {
        ...
        // Lấy 3 đơn đặt hàng gần đây nhất
        List<Order> ex6 = orders.stream()
                .sorted(Comparator.comparing(Order::getOrderDate).reversed())
                .limit(3)
                .collect(Collectors.toList());
        System.out.println("Danh sách 3 đơn đặt hàng gần đây nhất:");
        for (Order order : ex6) {
            System.out.println("ID: " + order.getId() + ", Ngày đặt hàng: " + order.getOrderDate());
        }
    }
}
