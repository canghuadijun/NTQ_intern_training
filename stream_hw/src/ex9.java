...
public class ex9 {
    public static void main(String[] args) {
        ...
        // Tính thanh toán trung bình cho đơn hàng được đặt vào ngày 14-3-2021
        LocalDate order_date = LocalDate.of(2021, 3, 14);

        OptionalDouble aver = orders.stream()
                .filter(order -> order.getOrderDate().equals(order_date))
                .flatMapToDouble(order -> order.getProducts().stream().mapToDouble(Product::getPrice)).average();

        if (aver.isPresent()) {
            System.out.println("Thanh toán trung bình cho đơn hàng được đặt vào ngày " + order_date + ": " + aver.getAsDouble());
        } else {
            System.out.println("Không có đơn hàng được đặt vào ngày " + order_date);
        }
    }
}
